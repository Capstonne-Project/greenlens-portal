...
import { AlertTriangle, ChevronLeft, ChevronRight, Inbox, Loader2, UserPlus } from 'lucide-react';
        ...
        <div className="min-w-0 flex-1 overflow-hidden rounded-2xl border border-emerald-100/80 bg-white/90 shadow-sm backdrop-blur dark:border-border dark:bg-card/90">
          {isPending ? (
            ...
            <ul className="divide-y divide-emerald-50 dark:divide-border">
              {items.map(item => (
                <li key={item.reportId} className="group flex">
                  <div
                    className={cn('w-1 shrink-0', severityRailClass(item.severity))}
                    aria-hidden
                  />
                  <div className="flex min-w-0 flex-1 flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between sm:py-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-sm font-bold text-emerald-900 dark:text-foreground">
                          {item.code}
                        </span>
                        <span className="text-xs font-medium text-muted-foreground">
                          {queueSeverityLabel(item.severity)}
                        </span>
                        {isSlaUrgent(item.slaResolveDueAt) && (
                      ...
                      <p className="mt-1 text-xs text-muted-foreground">
                        {item.categoryName} · Phường {item.wardCode} ·{' '}
                        {formatQueueRelativeTime(item.dispatchedAt)} · SLA{' '}
                        {formatCompanyDate(item.slaResolveDueAt)}
                      </p>
                    </div>
                    <button
                      type="button"
                      onClick={() => setAssignTarget(item)}
                      className="inline-flex shrink-0 items-center justify-center gap-2 rounded-xl bg-emerald-600 px-4 py-2 text-sm font-semibold text-white opacity-90 transition group-hover:opacity-100 hover:bg-emerald-700 sm:opacity-70"
                    >
