...
export interface Globe3DConfig {
  ...
  /** Center this lat/lng toward the camera on first render */
  initialViewLatLng?: { lat: number; lng: number };
  /** Marker default size */
  markerSize?: number;
...
/**
 * Y-rotation so the given coordinate faces the default camera (+Z).
 */
function computeGlobeRotationForLatLng(
  lat: number,
  lng: number,
): { x: number; y: number } {
  const point = latLngToVector3(lat, lng, 1);
  return { x: 0, y: -Math.atan2(point.x, point.z) };
}
...
function RotatingGlobe({
  ...
  const initialRotation = useMemo(() => {
    if (config.initialViewLatLng) {
      return computeGlobeRotationForLatLng(
        config.initialViewLatLng.lat,
        config.initialViewLatLng.lng,
      );
    }
    return config.initialRotation;
  }, [config.initialRotation, config.initialViewLatLng]);
...
type ResolvedGlobe3DConfig = Required<Omit<Globe3DConfig, 'initialViewLatLng'>> &
  Pick<Globe3DConfig, 'initialViewLatLng'>;

const defaultConfig: ResolvedGlobe3DConfig = {
  radius: 2,
  globeColor: "#1a1a2e",
