# SU26SE049 — Project Handbook (nguồn generate)

> **Bản dùng cho nộp báo cáo / overview dự án:** `docs/Overview.md` (đã lọc sạch, chỉ SU26SE049).  
> File này giữ để soạn thảo hoặc chạy `node scripts/generate-overview-md.mjs` tạo lại `Overview.md`.

# SU26SE049 — Project Handbook (Full Spec)
...
├── public/
├── docs/
│   └── PROJECT_HANDBOOK_SU26SE049.md   # tài liệu này
├── middleware.ts
├── next.config.mjs
...
**Không dùng** (so template Revoland cũ): `(hosting)`, `(saler)`, `(company)`; thay `property/` / `project/` bằng `report/`, `map/`, v.v. như bảng trên.
...
### 5.2 `middleware.ts` (thay thế rule Revoland cũ)
...
### 6.3 Ví dụ service (thay `fetchProperty` bằng `fetchReport`)
...
11. (Tuỳ chọn) Mục tiêu Lighthouse mobile ≥ 90 (performance / a11y) trên trang chính và bản đồ — tài liệu gốc: `docs/lighthouse-checklist.md` nếu repo thêm.
...
**Ghi chú SU26:** có thể đổi `--revo-header-height` thành `--app-header-height` khi dọn theme; không bắt buộc.
...
### 13.10 Đặt tên (rút từ blueprint)
...
4. Copy: `app/globals.css` từ **Phụ lục A** (hoặc mục 11.4 `PROJECT_STRUCTURE.md`); kèm `postcss.config.mjs`, `tsconfig`, `components.json`, ESLint/Prettier/Husky, `middleware.ts` (mục 5).
...
- `PROJECT_STRUCTURE.md` — bản mẫu **Revoland**; mục **§0–§19** (RSC, `lib/api`, hooks, `globals.css` §11.4) vẫn dùng song song.
- `SU26SE049_BusinessRules_v1.0.docx` — nguồn BR.
- `.cursor/rules/*.mdc` — nếu repo có Cursor rules.
- Tài liệu này: **`docs/PROJECT_HANDBOOK_SU26SE049.md`** (bản tổng hợp ưu tiên).
- Đồng bộ lại nội dung **Phụ lục A** từ `PROJECT_STRUCTURE.md` mục 11.4: `node scripts/fix-handbook-appendix.mjs` (chạy ở thư mục gốc repo).

---

_Bản tổng hợp: cấu trúc, thiết kế, rules, performance, security, cấu hình lần đầu — dùng làm chuẩn cho team SU26SE049._

---

## Phụ lục A: app/globals.css (chuẩn Tailwind v4)

Sao chép nguyên block sau vào `app/globals.css` khi init (đồng bộ mục 11.4 trong `PROJECT_STRUCTURE.md`).
